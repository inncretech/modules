...
public class FollowSourceDaoImpl extends GenericSourceShardDaoImpl<FollowSource, Long> implements FollowSourceDao {
  ...
  public Collection<? extends FollowSource> getFollowedSources(Integer shardId, Long userId) {

    Session sess = getCurrentSessionByShard(shardId);
    Query query = sess.createQuery("from FollowSource where followerId= :user_id").setParameter("user_id", userId);
    return query.list();
  ...
  @SuppressWarnings("unchecked")
  @ShardAware(shardStrategy = "shardid")
  public Collection<? extends FollowSource> getFollowersBySource(Integer shardId, Long sourceId) {
    Session sess = getCurrentSessionByShard(shardId);
    Query query = sess.createQuery("from FollowSource where sourceId= :source_id").setParameter("source_id", sourceId);
    return query.list();
