...
@Component
public class FollowTagDaoImpl extends GenericUserShardDaoImpl<FollowTag, Long>
		implements FollowTagDao {
	@Autowired
	...
	public Collection<? extends FollowTag> getfollowedTagsByUser(Long userId) {
		...
		Query query = getQuery(
				getIdGenService().getShardId(userId, ShardType.USER),
				"from FollowTag where followerId= :user_id");
	...
	public boolean doesUserFollowTag(long userId, long tagId) {
		Query query = getQuery(
				getIdGenService().getShardId(userId, ShardType.USER),
				"from FollowUser where userId= :userId" + " and tagId= :tagId"
	...
	public void unfollowTag(Long userId, Long tagId) {
		...
		Query query = getQuery(
				getIdGenService().getShardId(userId, ShardType.USER),
				"update FollowTag set record_status= :record_status"
