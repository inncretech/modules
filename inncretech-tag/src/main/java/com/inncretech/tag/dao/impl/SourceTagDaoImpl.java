...
@Component
public class SourceTagDaoImpl extends GenericSourceShardDaoImpl<SourceTag, Long> 
							  implements SourceTagDao{
	...
	public List<SourceTag> getTagsOfSource(Long sourceId) { 
		Query query = getQuery(getIdGenService().getShardId(sourceId, ShardType.SOURCE),
				"from SourceTag where sourceId= :sourceid"
	...
	public void removeTagFromSource(Long sourceId, Long tagId) {
		Query query = getQuery(getIdGenService().getShardId(sourceId, ShardType.SOURCE),
				"update SourceTag set record_status= :record_status"
